...
- (void) loadContent {
    ...
    if (!_label || !_slider) {
        ...
        _label.font = [UIFont fontWithName:@"Montserrat" size:18.0];
        _label.text = @"Slide";
...
- (void)layoutSubviews {
    ...
    _label.frame = CGRectMake(sliderWidth + 5.0,
                              CGRectGetMidY(self.bounds) - (labelSize.height / 2.0) - 3,
                              CGRectGetWidth(self.bounds) - sliderWidth - 5.0,
                              labelSize.height
                              );
...
- (void) setThumbColor:(UIColor *)color {
    [_slider setThumbImage:[UIImage imageNamed:@"btn_next_round"] forState:UIControlStateNormal];
}
