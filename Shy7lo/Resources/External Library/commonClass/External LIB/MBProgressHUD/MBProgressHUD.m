...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.alpha = 0.0f;
        self.color = [UIColor colorWithRed:7/255.0 green:109/255.0 blue:135/255.0 alpha:1];
...
- (void)show:(BOOL)animated {
    ...
    UIWindow *window = delegate1.window;
    
    UIView *loaderView = (UIView *)[window viewWithTag:2222];
    loaderView.hidden = NO;
    UIImageView *imgLoader;
    for(UIImageView *img in loaderView.subviews)
    {
        if(img.tag == 1111)
        {
            imgLoader = (UIImageView*)img;
        }
    }
    ...
    [imgLoader.layer addAnimation:fullRotation forKey:@"Spin"];
}
...
- (void)hide:(BOOL)animated
{
    ...
    [imgLoader.layer removeAnimationForKey:@"Spin"];
